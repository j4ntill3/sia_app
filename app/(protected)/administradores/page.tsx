...
const Administradores = () => {
  ...
  const fetchAdministradores = async () => {
    try {
      ...
      setAdministradores(administradoresData.data || []);
    } catch (err) {
  ...
  const handleDownloadCSV = async (exportAll: boolean = true) => {
    setIsExporting(true);
    try {
      const dataToExport = exportAll ? administradores : (searchQuery ? filteredData : paginatedData);

      const filename = exportAll
        ? "administradores-todos.csv"
        : searchQuery
          ? `administradores-filtrados.csv`
          : `administradores-pagina-${currentPage}.csv`;
      ...
      downloadCSV(
        ...
        filename,
        (administrador) => [
          administrador.empleado?.id || "",
          administrador.persona?.nombre || "",
          administrador.persona?.apellido || "",
          administrador.persona?.dni || "",
          administrador.empleado?.cuit || "",
          administrador.persona?.telefono || "",
          formatDateForCSV(administrador.empleado?.fecha_ingreso),
          administrador.persona?.correo || "",
          administrador.empleado?.fecha_egreso ? "Inactivo" : "Activo"
        ]
      );
    } catch (error) {
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
            <div className="w-full sm:w-auto">
              <SearchBar
                ...
                placeholder="Buscar administradores por nombre, DNI, CUIT, correo, teléfono..."
                  ...
                  <div className="absolute right-0 mt-2 w-64 bg-white rounded-lg shadow-lg border border-gray-200 z-20 overflow-hidden">
                    ...
                    >
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500">Todos los registros ({administradores.length})</p>
                      </div>
                    </button>
                    <button
                ...
                {paginatedData.length === 0 ? (
                  <tr>
                    <td colSpan={9} className="px-4 py-8 text-center text-gray-600">
                      {searchQuery ? "No se encontraron administradores con ese criterio de búsqueda" : "No hay administradores registrados"}
                    </td>
                  </tr>
                ) : (
                  paginatedData.map((administrador) => (
                    <AgenteItem
                      key={administrador.empleado.id}
