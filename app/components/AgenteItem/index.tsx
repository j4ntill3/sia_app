...
const AgenteItem: React.FC<AgenteItemProps> = ({ agente, onView }) => {
  ...
  return (
    <tr className="border-b font-sans hover:bg-gray-50">
      ...
      <td className="px-4 font-sans py-2">{empleado.CUIT}</td>
      <td className="px-4 font-sans py-2">
      ...
      <td className="px-4 font-sans py-2">
        <span
          className={`px-2 font-sans py-1 text-sm rounded-full ${
            empleado.eliminado
      ...
      <td className="px-4 font-sans py-2 text-center">
        <button
          onClick={() => onView(empleado.id)} // Llama a la función onView con el id del agente
          className="px-4 font-sans py-2 text-white bg-blue-500 rounded hover:bg-blue-600"
        >
          Ver
        </button>
      </td>
